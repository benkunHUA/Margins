...
# 索引键分配器：与 `SqliteIndexBackend` 共用同一张 `index_seq` 表
_ALLOCATE = text(
    "UPDATE index_seq SET next_id = next_id + :n "
    "WHERE id = 1 RETURNING next_id - :n AS start"
)
_ENSURE_SEQ = text(
    "INSERT INTO index_seq (id, next_id) VALUES (1, 0) "
...
class ChunkSqlRepository(ChunkRepository):
    ...
    async def add_many(self, chunks: Sequence[Chunk]) -> None:
        async with self._sf() as session:
            pending = [chunk for chunk in chunks if chunk.idx_id is None]
            if pending:
